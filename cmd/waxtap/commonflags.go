...
func bindPlayerExtractionFlags(f *pflag.FlagSet) {
	...
	f.String("client", "", "force one built-in client: web|ios|android_vr|visionos|web_embedded (conflicts with --profile-override; --player-context-url is tried first; avoid ios for audio downloads)")
